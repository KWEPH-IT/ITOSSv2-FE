...
type DashboardStatus = "Waiting for Action" | "On Process";

// NOTE: adjust these mappings to match the real raw values coming back
// from item.Status in your ticketing API.
...
const statusStyles: Record<
  DashboardStatus,
  { bg: string; color: string; iconBg: string; icon: React.ReactNode }
> = {
  "Waiting for Action": {
    bg: "#FCE8D4",
    color: "#B96A1F",
...
const TicketsForProcessing: React.FC = () => {
  const { ticket, loading } = useTickets(EMPTY_FILTERS);
  ...
  const tickets = useMemo(
    () =>
      allTickets.filter(
        (item) =>
          item.RequestType === 17 &&
          (item.Status === "Approved By IT Manager" ||
    ...
    () => [
      {
        ...
        value: statusCounts["Waiting for Action"],
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 16, color: "#222" }}>Tickets for Processing</span>
        ...
        >
          View all tickets <RightOutlined style={{ fontSize: 10 }} />
        </a>
      </div>
...
export default TicketsForProcessing;